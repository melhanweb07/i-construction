...
import type { RealEstateEnquiry } from "@/types";
...
export default function RealEstateEnquiryForm({ plotSlug }: { plotSlug?: string }) {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const payload: RealEstateEnquiry = {
      name,
      phone,
      email: String(form.get("email") || ""),
      preferredLocation: String(form.get("preferredLocation") || ""),
      requiredPlotSize: String(form.get("requiredPlotSize") || ""),
      budget: String(form.get("budget") || ""),
      purpose: String(form.get("purpose") || ""),
      preferredContactMethod: (form.get("preferredContactMethod") as RealEstateEnquiry["preferredContactMethod"]) || undefined,
      message: String(form.get("message") || (plotSlug ? `Interested in plot: ${plotSlug}` : "")),
    };

    setStatus("loading");
    try {
      const result = await submitPropertyEnquiry(payload);
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
        ...
        <Field label="Required Plot Size" name="requiredPlotSize" />
        <Field label="Budget" name="budget" />
        <Field label="Purpose" name="purpose" defaultValue={plotSlug ? `Enquiry for ${plotSlug}` : undefined} />
        <div className="relative">
          <select
            id="re-preferredContactMethod"
          ...
          >
            ...
            <option value="Call" className="bg-[#121415]">Call</option>
            <option value="WhatsApp" className="bg-[#121415]">WhatsApp</option>
            <option value="Email" className="bg-[#121415]">Email</option>
          </select>
          <label htmlFor="re-preferredContactMethod" className="pointer-events-none absolute left-4 top-2 text-[10px] uppercase tracking-[0.15em] text-[#A5A5A0]">
            Preferred Contact Method
          </label>
        </div>
      </div>
      <div className="relative">
        <textarea id="re-message" name="message" rows={4} placeholder="Message" className={`${inputClasses} resize-none`} />
        <label htmlFor="re-message" className={labelClasses}>
          Message
        </label>
      </div>
